  ...
  -- it then will spawn and route a 'convoy' to that location and if the convoy group arrives it will deploy a farp structure.
  -- All values are required to be entered.
  -- @param #FARPCREATOR self
  -- @param #string name - The name of this FARPCREATOR instance also used in the Menu name to identify.
  -- @param #string prefix - Client groups containing this will be able to access this functions menu
  -- @param #string convoytemplate - The mission editor template for the FARP convoy
  -- @param #string servicetemplate - The mission editor template for the Service vehicles for the Farp, remember this needs all your service units!
  -- @param #enum countryid - the country id that any units or farps spawned by this will be on ie country.id.USA
  -- @param #menu _missionmenu - the Menu that this farp will create menu options as a child of. 
  -- @return self
function FARPCREATOR:New(name,prefix,convoytemplate,servicetemplate,countryid)
  ...
  -- @param #integer targetradius - How close the convoy has to get the point the player requested it create the farp before it spawns.
  -- @return self
...
-- @param #boolen - True or False on the use.
-- @return self
      ...
      if v ~= nil then
         ...
         if  temptable.FarpServiceConvoy:IsAlive() then
           ...
           if Convoyhasarrived == true then
              ...
              -- it's not arrived so we don't care right now.
           end
         else
